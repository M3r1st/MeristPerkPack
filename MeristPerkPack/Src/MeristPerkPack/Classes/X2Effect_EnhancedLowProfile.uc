class X2Effect_EnhancedLowProfile extends X2Effect_Persistent;
    ...
    out array<ShotModifierInfo> ShotModifiers)
{
    ...
    if (`TACTICALRULES.VisibilityMgr.GetVisibilityInfo(Target.ObjectID, Attacker.ObjectID, VisInfo)
        && (!VisInfo.bVisibleGameplay || VisInfo.TargetCover != CT_None))
    {
        AimInfo.ModType = eHit_Success;
        AimInfo.Reason = FriendlyName;
        AimInfo.Value = `GetConfigInt("M31_EnhancedLowProfile_AimBonus");
        ShotModifiers.AddItem(AimInfo);

        CritInfo.ModType = eHit_Crit;
        CritInfo.Reason = FriendlyName;
        CritInfo.Value = `GetConfigInt("M31_EnhancedLowProfile_CritBonus");
