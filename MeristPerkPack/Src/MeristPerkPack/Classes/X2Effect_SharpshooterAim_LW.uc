...
function GetToHitModifiers(XComGameState_Effect EffectState, XComGameState_Unit Attacker, XComGameState_Unit Target, XComGameState_Ability AbilityState, class<X2AbilityToHitCalc> ToHitType, bool bMelee, bool bFlanking, bool bIndirectFire, out array<ShotModifierInfo> ShotModifiers)
{
    ...
    if (!bMelee)
    {
        ...
        ShotInfo_Hit.Value = `GetConfigInt("M31_SharpshooterAim_AimBonus");
        ShotModifiers.AddItem(ShotInfo_Hit);

        ShotInfo_Crit.ModType = eHit_Crit;
        ShotInfo_Crit.Reason = FriendlyName;
        ShotInfo_Crit.Value = `GetConfigInt("M31_SharpshooterAim_CritBonus");
...
static function X2Effect_SharpshooterAim_LW SharpshooterAimEffect()
{
    ...
    AimEffect.SetDisplayInfo(ePerkBuff_Bonus,
        `GetLocalizedString("M31_SharpshooterAim_FriendlyName"),
        `GetLocalizedString("M31_SharpshooterAim_BuffText"),
        "img:///UILibrary_PerkIcons.UIPerk_aim");

    AbilityCondition = new class'X2Condition_AbilityProperty';
    AbilityCondition.OwnerHasSoldierAbilities.AddItem('M31_Aim');
