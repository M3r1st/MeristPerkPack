...
static final function bool WasUnitFullyProtected(const XComGameState_Unit OldUnitState, const XComGameState_Unit NewUnitState)
{
    ...
    NewUnitState.GetUnitValue(default.TemplarShield_ShieldValueName, ShieldRemainingUV);
...
static final function bool WasShieldFullyConsumed(const XComGameState_Unit OldUnitState, const XComGameState_Unit NewUnitState)
{
    ...
    return int(ShieldRemainingUV.fValue) <= 0;
}
...
private static function EventListenerReturn TemplarShield_OnAbilityActivated(Object EventData, Object EventSource, XComGameState NewGameState, Name Event, Object CallbackData)
{
    ...
    if (TargetUnit != none && TargetUnit.IsUnitAffectedByEffectName(class'X2Effect_MeristTemplarShield'.default.EffectName))
    {
...
private static function ReplaceHitAnimation_PostBuildVis(XComGameState VisualizeGameState)
{
    ...
    foreach FindActions(FindAction)
    {
        ...
        if (!OldUnitState.IsUnitAffectedByEffectName(class'X2Effect_MeristTemplarShield'.default.EffectName)) // Check the old unit state specifically, as the attack could have removed the effect from the target.
            continue;
        ...
        if (class'X2Effect_MeristTemplarShield'.static.WasUnitFullyProtected(OldUnitState, NewUnitState))
        {
